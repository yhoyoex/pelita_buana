...
handleAjaxMode=function(c){
  var h=c.emptyHtml?c.emptyHtml:"<div class='error'><div class='error-code m-b-10'>404</div><div class='error-content'><div class='error-message'>We couldn't find it...</div><div class='error-desc m-b-30'>The page you're looking for doesn't exist. <br />Perhaps, there pages will help find what you're looking for.</div></div></div>",
  ...
  function a(e,a,t){
    var n,o,i, cp;
    cp = e.substring(e.lastIndexOf('/')+1),
    Pace.restart(),
    u(!1),
    ...
    n='#sidebar [data-toggle="ajax"][href="'+e+'"]',0!==$(n).length&&($("#sidebar li").removeClass("active"),
    $(n).closest("li").addClass("active"),$(n).parents().addClass("active")),CLEAR_OPTION&&(App.clearPageOption(CLEAR_OPTION),CLEAR_OPTION=""),
    t||(o=e.replace("#",""),(i=window.navigator.userAgent.indexOf("MSIE "))&&0<i&&i<9?window.location.href=o:history.pushState("","","#"+cp));
    var s="#content",
    l=e.replace("#",""),r=c.ajaxType?c.ajaxType:"GET",d=c.ajaxDataType?c.ajaxDataType:"html";a&&(d=$(a).attr("data-type")?$(a).attr("data-type"):d,targetDataDataType=$(a).attr("data-data-type")?$(a).attr("data-data-type"):d),
    $.ajax({
      url:l,
      type:r,
      dataType:d,
    ...
    done(function(){u(!0),$("html, body").animate({scrollTop:0},0),
    App.initComponent()})}""===(e=window.location.hash?window.location.hash:e)?$("#content").html(h):a(e,"",!0),$(window).on("hashchange",function(){window.location.hash&&a(window.location.hash,"",!0)}),
